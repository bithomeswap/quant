...
def get_technical_indicators(df):  # 定义计算技术指标的函数
    try:
        ...
        for n in range(1, 7):
            ...
            df[f'+1{n*2}周期昨日资金贡献rsi'] = talib.RSI(
                df['昨日资金贡献+1'], timeperiod=n*2)
            df[f'abs{n*2}周期昨日资金贡献rsi'] = talib.RSI(
                df['昨日资金贡献abs'], timeperiod=n*2)

            df[f'{n*2}周期昨日涨跌幅rsi'] = talib.RSI(df['昨日涨跌幅'], timeperiod=n*2)
            df[f'+1{n*2}周期昨日涨跌幅rsi'] = talib.RSI(df['昨日涨跌幅+1'], timeperiod=n*2)
            df[f'abs{n*2}周期昨日涨跌幅rsi'] = talib.RSI(
                df['昨日涨跌幅abs'], timeperiod=n*2)
