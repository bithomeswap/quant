...
def get_indicators(df):  # 定义计算技术指标的函数
    try:
        ...
        # 删除最高价和最低价为负值的数据
        df =df.drop(df[(df['最高'] < 0) | (df['最低'] < 0)].index, inplace=True)
...
async def process_data(name):
    ...
    print(f'{name}数据读取成功')
    # 按照“代码”列进行分组并计算技术指标
    grouped = data.groupby('代码', group_keys=False).apply(get_indicators)
    # 分组并计算指标排名
    grouped = grouped.groupby(['日期'], group_keys=False).apply(paiming)
    # 当日符合要求的标的推送钉钉
    tradelist(grouped, name)
    # 连接MongoDB数据库并创建新集合
...
async def process_all_data():
    tasks = []
    for name in names:
        # 将传入参数与异步函数运行绑定为一个协程任务，并添加到任务列表中
        tasks.append(asyncio.create_task(process_data(name)))
    # 使用 asyncio 库的 wait 函数等待所有任务完成
    await asyncio.wait(tasks)
# 创建事件循环对象
loop = asyncio.get_event_loop()
# 在事件循环中运行协程函数
loop.run_until_complete(process_all_data())
print("全部数据处理完成！")
