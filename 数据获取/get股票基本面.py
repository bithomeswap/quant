...
for name in names:
    try:
        ...
        for code in df['代码']:
            ...
            for typename in ["总市值", "市盈率(TTM)", "市净率", "市现率"]:
                k_value = ak.stock_zh_valuation_baidu(symbol=code,  indicator=typename, period="全部")
                k_value.rename(columns={'date': '日期', 'value': f'{typename}'}, inplace=True)
