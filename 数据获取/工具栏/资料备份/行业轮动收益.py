...
def se(choosename, name, df):
    if choosename == '交易':
        # 获取所有不重复日期，理论上应该是计算当日的数据，这里为了统计方便使用的未来函数
        df = df[df[f'过去{b}日涨跌{a/100}金叉'] == 1].copy()
        m = 0.000  # 设置手续费
        n = 6  # 设置持仓周期
    return df, m, n
            ...
            for b in (5, 20):
                for a in range(-5, 6):
                    try:
                        ...
                        m = 0.001  # 设置手续费
                        n = 6  # 设置持仓周期
                        df, m, n = se('交易', name, df)
                        df.to_csv(f'{name}_金叉{a}_时间{b}交易细节.csv',
                                  index=False)  # 输出交易细节
                        if updown == '盘中波动':
                            ...
                            for date in dates:
                                ...
                                daydf = daydf.sort_values(
                                    by='日期')    # 以日期列为索引,避免计算错误
                            ...
                            if not os.path.exists(path):
                                os.makedirs(path)
                            result_df.to_csv(
                                f'{path}/{name}_{updown}_金叉{a}_时间{b}资金波动.csv', index=False)
                        if updown == '资产收益':
                            result_df = pd.DataFrame({})
                            ...
                            if not os.path.exists(path):
                                os.makedirs(path)
                            result_df.to_csv(
                                f'{path}/{name}_{updown}_金叉{a}_时间{b}理论收益.csv', index=False)
                            print('任务已经完成！')
