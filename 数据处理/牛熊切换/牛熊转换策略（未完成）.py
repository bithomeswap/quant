...
# 设置持仓周期
# n 表示持仓周期，例如当天买入的股票需要一直持有 n 天，n 天后卖出
n = 1
...
for date, group in df.groupby('日期'):
    ...
    if group.empty:
        ...
        # 计算当日的收益率
        daily_return = selected_stocks[f'{n}日后总涨跌幅（未来函数）'].mean()*(1-m)/100

        # 对后n天的每日涨跌幅与手续费进行扣除，并更新持仓周期
        for i in range(1, n+1):
            group[f'{i}日后总涨跌幅（未来函数）'] = (
                group[f'{i}日后总涨跌幅（未来函数）']*(1-m)/100).apply(lambda x: x if x > 0 else 0)
        group = group[group[f'{n}日后总涨跌幅（未来函数）'] > 0]
        group[f'{n}日后总涨跌幅（未来函数）'] = group[f'{n}日后总涨跌幅（未来函数）'] / 100

    # 记录当日执行策略和当日收益率，并将当日收益率累加到资金余额
    df_strategy = pd.concat([df_strategy, pd.DataFrame(
        {'日期': [date], '执行策略': [df_mean[df_mean['日期'] == date]['策略'].iloc[0]]})])
